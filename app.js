...
async function init(){
  ["orderDate","productionDate","taskDate"].forEach(id=>$(id).value=today());
  bind(); await loadSettings(); await loadEmployees(); await loadEntries(); await loadCustomers(); await loadOrders(); await loadProduction(); await loadTasks(); await loadInventory();
  renderAll();
}
function bind(){
